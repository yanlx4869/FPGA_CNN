    ...
    for user in result:
        if (username == user[1]) and (password == user[2]):
            ...
            break
             ...
             flag = False
             # return "用户名或密码错误"
             return render_template("modify_email.html", status="error")
    ...
    if flag:
     # 执行修改操作
        if r.get('scode') == security_code:
            ...
            if count > 0:
              # return "修改成功！"
              return render_template("modify_email.html",status="ok")
            else:
              # print("修改失败！")
              return render_template("modify_email.html",status="failed")
        else:
            # return "验证码错误"
            return render_template("modify_email.html",status="error1")
